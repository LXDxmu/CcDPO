...
from trl.trainer import DPOTrainer, VDPOTrainer, SVCOTrainer
...
class LLaVADPOTrainer_Vision(VDPOTrainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            super(LLaVADPOTrainer_Vision, self)._save(output_dir, state_dict)


class LLaVADPOTrainer_SVCO(SVCOTrainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            super(LLaVADPOTrainer_SVCO, self)._save(output_dir, state_dict)