...
from llava.train.llava_trainer import LLaVADPOTrainer, LLaVADPOTrainer_Vision, LLaVADPOTrainer_SVCO
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_symmetrical_loss: bool = False
    dpo_token_weight: float = 1
    dpo_token_weighted: bool = False
    tok_beta: float = 0.1
    use_tdpo: bool = False
    abs_weight: float = 0.25
    vdpo_weight: float = 1
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if has_image and "<image>" in sentence["value"]:
                ...
                segments = sentence["value"].split("<image>")
                for idx, seg in enumerate(segments):
                    if idx > 0:
                        _input_id += [IMAGE_TOKEN_INDEX] + nl_tokens
                    if seg.strip():
                        _input_id += tokenizer(seg).input_ids
...
class DPODataset_symmetrical(Dataset):
    """Dataset for DPODataset fine-tuning."""

    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        super(DPODataset_symmetrical, self).__init__()
    ...
    def process_image(self, image_file, diffusion=False,diffusion_image_file=False):
        ...
        if self.data_args.use_image_type == 'diffusion' and diffusion:
            diffusion_image = add_diffusion_noise(image, self.data_args.diffusion_step)
            diffusion_image_size = image_size
        elif self.data_args.use_image_type == 'all_diffusion':
            diffusion_image = add_diffusion_noise(image, self.data_args.diffusion_step)
            diffusion_image_size = image_size
        elif (self.data_args.use_image_type == 'otherorder' or self.data_args.use_image_type == 'otherimage') and diffusion_image_file:
            diffusion_image = Image.open(os.path.join(image_folder, diffusion_image_file)).convert("RGB")
            diffusion_image_size = diffusion_image.size
            # if diffusion:
            #     diffusion_image = add_diffusion_noise(image, self.data_args.diffusion_step)
            #     diffusion_image_size = image_size
            # else:
            if self.data_args.image_aspect_ratio == "highres":
                diffusion_image = process_highres_image(diffusion_image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            elif self.data_args.image_aspect_ratio == "anyres" or "anyres" in self.data_args.image_aspect_ratio:
                diffusion_image = process_anyres_image(diffusion_image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            elif self.data_args.image_aspect_ratio == "crop_split":
                diffusion_image = process_highres_image_crop_split(diffusion_image, self.data_args)
            elif self.data_args.image_aspect_ratio == "pad":
                def expand2square(pil_img, background_color):
                    width, height = pil_img.size
                    if width == height:
                        ...
                        result.paste(pil_img, ((height - width) // 2, 0))
                        return result
                diffusion_image = expand2square(diffusion_image, tuple(int(x * 255) for x in processor.image_mean))
                diffusion_image = processor.preprocess(diffusion_image, return_tensors="pt")["pixel_values"][0]
            else:
                diffusion_image = processor.preprocess(diffusion_image, return_tensors="pt")["pixel_values"][0]
        elif self.data_args.use_image_type == 'diffusion_otherimage' and diffusion_image_file:
            if diffusion:
                diffusion_image = add_diffusion_noise(image, self.data_args.diffusion_step)
                diffusion_image_size = image_size
            elif diffusion_image_file.startswith("multi_image_vdpo"):
                diffusion_image = Image.open(os.path.join(image_folder, diffusion_image_file)).convert("RGB")
                diffusion_image_size = diffusion_image.size
                if self.data_args.image_aspect_ratio == "highres":
                    diffusion_image = process_highres_image(diffusion_image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            ...
            #diffusion_image = add_diffusion_noise(image, self.data_args.diffusion_step)
            #diffusion_image_size = image_size
        diffusion_image = diffusion_image.reshape(-1, 3, diffusion_image.shape[-2], diffusion_image.shape[-1])
        return image, image_size, "image", diffusion_image, diffusion_image_size, "image"

    def process_rejected_image(self, image_file, diffusion=False):
        image_folder = self.data_args.image_folder
        processor = self.data_args.image_processor
        # print(f"\n\nInspecting the image path, folder = {image_folder}, image={image_file}\n\n")
        try:
        ...
        if self.data_args.image_aspect_ratio == "highres":
            ...
            image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
        if self.data_args.use_image_type == 'diffusion' and diffusion:
            image = add_diffusion_noise(image, self.data_args.diffusion_step)
        image = image.reshape(-1, 3, image.shape[-2], image.shape[-1])
        return image, image_size, "image"

    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        sources = self.list_data_dict[i]
        chosen = {'from': 'gpt', 'value': sources['chosen']}
        rejected = {'from': 'gpt', 'value': sources['rejected']}
        source = {
            'image': sources['image'],
            'random_image': sources['image'],
            "chosen": chosen,
            "rejected": rejected,
            "idx": sources['id'],
        }
        ...
        if "image" in sources[0]:
            ...
            diffusion_image_list = self.list_data_dict[i].get('diffusion_image', [])
            if type(image_file) is list:
                if self.data_args.use_image_type == 'diffusion' or self.data_args.use_image_type == 'all_diffusion':
                    image = [self.process_image(f, diffusion=f in diffusion_image_list, diffusion_image_file=None) for f in image_file]
                else:
                    if diffusion_image_list:
                        image = [self.process_image(f, diffusion=f in diffusion_image_list, diffusion_image_file=diffusion_image) for f, diffusion_image in zip_longest(image_file, diffusion_image_list,fillvalue="None")]
                    else:
                        image = [self.process_image(f, diffusion=False, diffusion_image_file=None) for f in image_file]
                #image = [self.process_image(f) for f in image_file]
                #diffusion_image = [self.process_rejected_image(f,diffusion=f in diffusion_image_list) for f in image_file]
            else:
                image = [self.process_image(image_file, diffusion=image_file in diffusion_image_list)]
                #image = [self.process_image(image_file)]
                #diffusion_image = [self.process_rejected_image(image_file, diffusion=image_file in diffusion_image_list)]
        ...
        prompt = data_dict['prompt']
        source['question'] = {'from': 'human', 'value': prompt}
        source['question_noimg'] = {'from': 'human', 'value': prompt.replace('<image>\n','')}
        win_conv = copy.deepcopy([source['question'], source["chosen"]])
        rej_conv = copy.deepcopy([source['question'], source["rejected"]])
        no_img_win_conv = copy.deepcopy([source['question_noimg'], source["chosen"]])
        no_img_rej_conv = copy.deepcopy([source['question_noimg'], source["rejected"]])

        diffusion_idx = self.list_data_dict[i].get('diffusion_idx', [])
        causal_flag=self.list_data_dict[i].get('causal', None)
        ...
        win_data_dict = dict(input_ids=win_data_dict["input_ids"][0],
                             ...
                             idx=self.list_data_dict[i]['id'],
                             causal=causal_flag,
                             diffusion_idx=diffusion_idx)
        
        no_img_rej_data_dict = preprocess([no_img_rej_conv], self.tokenizer, has_image=False)
        no_img_rej_data_dict = dict(input_ids=no_img_rej_data_dict["input_ids"][0],
                             labels=no_img_rej_data_dict["labels"][0])
        no_img_win_data_dict = preprocess([no_img_win_conv], self.tokenizer, has_image=False)
        no_img_win_data_dict = dict(input_ids=no_img_win_data_dict["input_ids"][0],
                             labels=no_img_win_data_dict["labels"][0])


        # image exist in the data
        if 'image' in source:
            rej_data_dict['image'] = win_data_dict['image'] = image
            #rej_data_dict['image_sizes'] = win_data_dict['image_sizes'] = [im[1] for im in image]
            #rej_data_dict['modalities'] = win_data_dict['modalities'] = [im[2] for im in image]
            #rej_data_dict['diffusion_image'] = win_data_dict['diffusion_image'] = diffusion_image
            #rej_data_dict['diffusion_image_sizes'] = win_data_dict['diffusion_image_sizes'] = [im[1] for im in diffusion_image]
            #rej_data_dict['diffusion_modalities'] = win_data_dict['diffusion_modalities'] = [im[2] for im in diffusion_image]
        ...
        return rej_data_dict, win_data_dict, no_img_rej_data_dict, no_img_win_data_dict
...
#
#             rejected_image= [instance["rejected_image"] for instance in features]
#
#             padded_batch["rejected_image_sizes"] = [im[1] for im_list in rejected_image for im in im_list]
#             padded_batch["rejected_modalities"] = [im[2] for im_list in rejected_image for im in im_list]
#             rejected_image = [im[0] for im_list in rejected_image for im in im_list]
#
#             # import pdb;pdb.set_trace()
#
#             padded_batch["images"] = images
#             padded_batch["rejected_image"] = rejected_image
#             #print(padded_batch["images"])
#             #print(len(padded_batch["images"]))
...
def make_dpo_data_module_symmetrical(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    train_dataset = DPODataset_symmetrical(tokenizer=tokenizer,
                               data_path=data_args.data_path,
                               data_args=data_args)
    print(f'Train data size is {len(train_dataset)}', flush=True)
    data_collator = DataCollatorForDPODataset_symmetrical(
        tokenizer=tokenizer,
    )
    return dict(train_dataset=train_dataset,
                eval_dataset=None,
...
def concate_pad_four(tensorA, tensorB, tensorC, tensorD, padding_value):
    out = torch.nn.utils.rnn.pad_sequence(
...
class DataCollatorForDPODataset(object):
    ...
    def SFT_collator_fn(self, instances, pad_token_id):
        ...
        if 'image' in instances[0]:
            ...
            batch['diffusion_image'] = [im[3] for im_list in images for im in im_list]
            batch['diffusion_image_sizes'] = [im[4] for im_list in images for im in im_list]
            batch['diffusion_modalities'] = [im[5] for im_list in images for im in im_list]

            #images = [instance['image'] for instance in instances]
            #diffusion_images = [instance['diffusion_image'] for instance in instances]
            #batch['image_sizes'] = [im[1] for im_list in images for im in im_list]
            #batch['modalities'] = [im[2] for im_list in images for im in im_list]
            #batch['diffusion_image_sizes'] = [im[1] for im_list in diffusion_images for im in im_list]
            #batch['diffusion_modalities'] = [im[2] for im_list in diffusion_images for im in im_list]
            #batch['images'] = [im[0] for im_list in images for im in im_list]
            #batch['diffusion_image'] = [im[0] for im_list in diffusion_images for im in im_list]
    ...
    def preference_collator_fn(self, instances, pad_token_id):
        ...
        batch = dict(
            ...
            diffusion_image_sizes=win_batch['diffusion_image_sizes'],
            diffusion_modalities=win_batch['diffusion_modalities'],
            idx=win_instances[0]['idx'],
            causal=win_instances[0]['causal'],
            diffusion_idx=win_instances[0]['diffusion_idx']
        )
...
@dataclass
class DataCollatorForDPODataset_symmetrical(object):
    tokenizer: transformers.PreTrainedTokenizer
    ...
    def preference_collator_fn(self, instances, pad_token_id):
        ...
        rej_instances, win_instances, no_img_rej_instances, no_img_win_instances = list(zip(*instances))
        rej_batch = self.SFT_collator_fn(rej_instances, pad_token_id)
        win_batch = self.SFT_collator_fn(win_instances, pad_token_id)
        no_img_rej_batch = self.SFT_collator_fn(no_img_rej_instances, pad_token_id)
        no_img_win_batch = self.SFT_collator_fn(no_img_win_instances, pad_token_id)

        concatenated_input_ids = concate_pad(win_batch['input_ids'], rej_batch['input_ids'], pad_token_id)
        concatenated_labels = concate_pad(win_batch['labels'], rej_batch['labels'], -100)
        concatenated_attention_mask = concatenated_input_ids.ne(pad_token_id)

        no_img_concatenated_input_ids = concate_pad(no_img_win_batch['input_ids'], no_img_rej_batch['input_ids'], pad_token_id)
        no_img_concatenated_labels = concate_pad(no_img_win_batch['labels'], no_img_rej_batch['labels'], -100)
        no_img_concatenated_attention_mask = no_img_concatenated_input_ids.ne(pad_token_id)

        concatenated_input_ids_4 = concate_pad_four(win_batch['input_ids'], rej_batch['input_ids'],
                                                     win_batch['input_ids'], rej_batch['input_ids'], pad_token_id)
        concatenated_labels_4 = concate_pad_four(win_batch['labels'], rej_batch['labels'], win_batch['labels'], rej_batch['labels'], -100)
        concatenated_attention_mask_4 = concatenated_input_ids_4.ne(pad_token_id)

        batch = dict(
            concatenated_input_ids=concatenated_input_ids,
            concatenated_labels=concatenated_labels,
            concatenated_attention_mask=concatenated_attention_mask,
            no_img_concatenated_input_ids=no_img_concatenated_input_ids,
            no_img_concatenated_labels=no_img_concatenated_labels,
            no_img_concatenated_attention_mask=no_img_concatenated_attention_mask,
            concatenated_input_ids_4=concatenated_input_ids_4,
            concatenated_labels_4=concatenated_labels_4,
            concatenated_attention_mask_4=concatenated_attention_mask_4,
            win_input_ids=win_batch['input_ids'],
            rej_input_ids=rej_batch['input_ids'],
            images=win_batch['images'],
            image_sizes=win_batch['image_sizes'],
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        # # '/opt/ml/input/data_2/lxd/youtu/CHiP_ov/ckpt/llava-onevision-qwen2-7b-ov_dpo-beta0.1-epoch1_sft',
        # # '/opt/ml/input/data_2/lxd/youtu/CHiP_ov/ckpt/llava-onevision-qwen2-7b-ov_dpo-beta0.1-epoch1_sft_cot'
        # # '/opt/ml/input/data_2/lxd/youtu/CHiP_ov/ckpt/llava-onevision-qwen2-7b-ov_tdpo-beta0.1-epoch1_1234_chip_code/checkpoint-100'
        # '/opt/ml/input/data_2/lxd/youtu/CHiP_ov/ckpt/llava-onevision-qwen2-7b-ov_tdpo-beta0.1-epoch1_1234_stage1_otherimage_bbox_point_11k_final_prefix_vdpofenli'
        # )
        # model = model.merge_and_unload()
        # model.save_pretrained("/opt/ml/input/data_2/lxd/model_pth/llava-onevision-qwen2-7b-ov-stage1-1234-ckpt100-stage2-bbox_point_11k-epoch1")
        # rank0_print("save checkponit...")
            ...
            if "lm_head" in name or "embed_tokens" in name:
                if hasattr(module, "weight"):
                    if training_args.bf16 and module.weight.dtype == torch.float32:
                        module = module.to(torch.bfloat16)
    if training_args.use_symmetrical_loss:
        data_module = make_dpo_data_module_symmetrical(tokenizer=tokenizer, data_args=data_args)
    else:
        data_module = make_dpo_data_module(tokenizer=tokenizer, data_args=data_args)

    trainer = LLaVADPOTrainer_SVCO(
        model,
        ref_model,
        args=training_args,
